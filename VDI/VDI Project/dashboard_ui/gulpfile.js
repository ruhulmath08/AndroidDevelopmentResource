...
frontendConfig = {
  entry: [
    'babel-polyfill',
    'webpack-dev-server/client?http://' + require("ip").address() + ':3004/',// for dev
    'webpack/hot/only-dev-server',
    './app/index.js'
  ],
//'webpack-dev-server/client?http://' + require("ip").address() + ':3002/', for dev
  module: {
    loaders: [{
      test: /\.jsx?$/,
...
gulp.task('backend-watch', function(done) {
  ...
  var cmd = './node_modules/.bin/nodemon ./src/index.js --exec babel-node'
  var obj = {
    options: {
      cwd: __dirname
    },
...
gulp.task('frontend-watch', function() {
  ...
  new WebpackDevServer(webpack(frontendConfig), {
    ...
    contentBase: 'dist/'
  }).listen(process.env.DEV_PORT_UI, function(err, status) {
    if(err) {
      console.log('Error -> Frontend ==> ', err);
    } else {
      console.log('webpack dev server listening at localhost:->',process.env.DEV_PORT_UI);
    }
  })
})
